...
* Fragen                                                             :ignore:
* TODO [4/11] Noch zu tun:                                           :ignore:
** TODO Samplerate für jede Messung rausfinden
** DONE layer.png
   CLOSED: [2019-10-10 Do 16:09]
** TODO Auschaltstrom Plot
** TODO Einleitung magnetophoretische Mobilität
** TODO DPI
** DONE minted
...
* Header                                                             :ignore:
   ...
   # Workaround für grffile  Fehler:
   #+latex_header: \makeatletter
   #+latex_header: \def\set@curr@file#1{%
   #+latex_header: \begingroup
   #+latex_header: \escapechar\m@ne
   #+latex_header: \xdef\@curr@file{\expandafter\string\csname #1\endcsname}%
   #+latex_header: \endgroup
   #+latex_header: }
   #+latex_header: \def\quote@name#1{"\quote@@name#1\@gobble""}
   #+latex_header: \def\quote@@name#1"{#1\quote@@name}
   #+latex_header: \def\unquote@name#1{\quote@@name#1\@gobble"}
...
Durch Helium-Ionenbeschuss kann der EB-Effekt eines Schichtsystems verändert werden. So kann zum Beispiel die Richtung des Austauschverschiebungsfeldes lokal umgekehrt werden cite:mougin_local_2001. Dies ermöglicht magnetische Strukturierung von EB-Systemen, zum Beispiel in einer /in-plane/ Anisotropie, welche zu /head-to-head/ und /tail-to-tail/-Domänenwänden führt, womit wiederum Transport von superparamagnetischen Partikeln realisiert werden kann cite:holzinger_directed_2015. 
...
Um den zeitlichen Verlauf des Magnetfeldes zu messen, wurde der Messkopf eines Teslameters auf dem Probentisch zwischen den Helmholtzspulen platziert. Das FM210 Teslameter der /Projekt Elektronik GmbH/ besitzt einen BNC-Anschluss, über welchen widerum ein DAQ-Gerät an einem AI angeschlossen wurde, um den zeitlichen Verlauf des Magnetfeldes aufzuzeichnen.
...
#+begin_src python :session strom :exports none :results output
  ...
  for i, data in enumerate(datalist):
      sr = 48000
      end = np.argmax(data>0.018)
      data = data[end:end+10*(sr+20)]
      datalist[i] = data

  fig, ax = plt.subplots(2,2, sharex=True)
  ax[0,0].plot(datalist[0], '.', markersize=4)
  ax[0,1].plot(datalist[2], '.', markersize=4)
  ax[1,0].plot(datalist[1], '.', markersize=4)
...
#+begin_src python :session strom :results output
  ...
  avglist = []
  for i, data in enumerate(datalist):
      periodic = data.reshape(n_periods, sr+20)
...
#+begin_src python :session strom :results output
  ...
  try:
      popt, pcov = curve_fit(I, x, avglist[2][:cut], p0=(3,R_z,L_z), bounds=(0,[20,20,5]), method="trf")
  except RuntimeError as e:
      print(e)
  plt.clf()
  plt.plot(avglist[0][:cut], '.', markersize=3)
  plt.plot(x, I(x, *popt), 'r--')
...
#+begin_src python :session mag :results file
  import matplotlib
  matplotlib.use("agg")
  import matplotlib.pyplot as plt
  import numpy as np
...
#+name: fig-mag
#+caption: Zeitlicher Verlauf des Magnetfeldes bei einer angelegten Rechteckspannung. Links ist das Magnetfeld der x-Spulen dargestellt, rechts das der z-Spulen.
#+attr_latex: :width \textwidth :placement [H]
[[file:img/mag.png]]
...
#+name: fig-mag_avg
#+caption: Über alle zehn Perioden gemittelte Messwerte des Magnetfeldes für x-Spule (links) und z-Spule (rechts).
#+attr_latex: :width \textwidth :placement [H]
...
#+name: fig-mag_anfang
#+caption: Verlauf des Magnetfeldes kurz nach Beginn der gemittelten Periode. Links für x-Spule, rechts für z-Spule.
#+attr_latex: :width \textwidth :placement [H]
...
# Trigger Camera
* Anhang
** DONE Python-Script zur Messung von Stromstärke und Magnetfeld: label:anh-messung
