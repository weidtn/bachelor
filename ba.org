...
* Fragen                                                             :ignore:
* TODO [1/10] Noch zu tun:                                           :ignore:
** TODO REIHENFOLGE FÜR FITS!!
...
#+begin_src python :session strom :results output
  ...
  for i, data in enumerate(avglist):
      ...
      popt, pcov = curve_fit(I, x, data[:cut], p0=(1,R_z,L_z), bounds=(0,[10,25,5]), method="trf")
      if i < 2:
          ax[0,i].plot(data[:cut])
          ax[0,i].plot(I(x,*popt), 'r--', label=f"U={popt[0]}")
          ax[0,i].legend()
      else:
          ax[1,i-2].plot(data[:cut])
          ax[1,i-2].plot(I(x,*popt), 'r--', label=f"U={popt[0]}")
          ax[1,i-2].legend()
