...
** TODO Strom fit
** TODO STROM PLOTS!!! RAW und AVERAGE
** TODO Diskussion:
   - Tracking
...
** TODO flacher = bessere voraussagbarkeit aber langsamere Partikel
...
#+RESULTS:
: C:\Users\exp4-AGE1-106\AppData\Roaming\Python\Python37\site-packages\matplotlib\pyplot.py:514: RuntimeWarning: More than 20 figures have been opened. Figures created through the pyplot interface (`matplotlib.pyplot.figure`) are retained until explicitly closed and may consume too much memory. (To control this warning, see the rcParam `figure.max_open_warning`).
...
:   File "c:/Users/EXP4-A~1/AppData/Local/Temp/babel-K5d1aF/python-VHAGmo", line 39, in <module>
:     popt, pcov = curve_fit(I_fixed_x, x, data[:cut], p0=(10), bounds=(0,50), method="trf")
: NameError: name 'I_fixed_x' is not defined
...
#+begin_src python :session strom :results output
  ...
  ax[1,1].plot(t, I(x, 0.317*(R_z), R_z, L_z), label="Berechnet")
  ax[1,1].legend()
  ax[1,0].set_xlabel("Zeit [s]")
  ax[1,1].set_xlabel("Zeit [s]")
  ax[0,0].set_ylabel("$I$ [A]")
...
#+begin_src python :session :results output
  ...
  traj6 = np.load(f"./code/3796x_10488z_{crop}.npy")[100:-200]
  traj5 = np.load(f"./code/4743x_13106z_{crop}.npy")[100:-200]
  traj4 = np.load(f"./code/6326x_17475z_{crop}.npy")[100:-200]
  traj3 = np.load(f"./code/9486x_26213z_{crop}.npy")[100:-200]
  traj2 = np.load(f"./code/23722x_65540z_{crop}.npy")[100:-200]
  traj1 = np.load(f"./code/106215x_156678z_{crop}.npy")[100:-200]
  ...
  t0 = np.linspace(0,1,len(traj_real_0))
  t1 = np.linspace(0,1,len(traj_real_1))
  t2 = np.linspace(0,1,len(traj_real_2))
  t3 = np.linspace(0,1,len(traj_real_3))
  t4 = np.linspace(0,1,len(traj_real_4))
  t5 = np.linspace(0,1,len(traj_real_5))
  traj6 = (traj6 - min(traj6)) 
  traj5 = (traj5 - min(traj5)) / max(traj5)
  traj4 = (traj4 - min(traj4)) / max(traj4)
  traj3 = (traj3 - min(traj3)) / max(traj3)
  traj2 = (traj2 - min(traj2)) / max(traj2)
  traj1 = (traj1 - min(traj1)) / max(traj1)
  ...
  axes[0,0].plot(time, traj1, color="C0")
  axes[0,1].plot(time, traj2, color="C1")
  axes[0,2].plot(time, traj3, color="C2")
  axes[0,3].plot(time, traj4, color="C3")
  axes[0,4].plot(time, traj5, color="C4")
  axes[0,5].plot(time, traj6, color="C5")
  axes[1,0].plot(t0, traj_real_0, color="C0")
  axes[1,1].plot(t1, traj_real_1, color="C1")
  axes[1,2].plot(t1, traj_real_2, color="C2")
  axes[1,3].plot(t1, traj_real_3, color="C3")
  axes[1,4].plot(t1, traj_real_4, color="C4")
  axes[1,5].plot(t1, traj_real_5, color="C5")
...
#+begin_src python :session magnitude :results output
  ...
  for idx in range(len(slope1)):
      if idx < 3:
            ...
            axes[0,idx].scatter(t, mag[idx], s=0.1, label = "magnitude")
            axes[0,idx].grid()
      else:
            axes[1,idx-3].scatter(t, x_trap[idx], s=0.1, label = "x")
            axes[1,idx-3].scatter(t, z_trap[idx], s=0.1, label = "z")
            axes[1,idx-3].scatter(t, mag[idx], s=0.1, label = "magnitude")
            axes[1,idx-3].grid()
...
:END:
#+name: fig-betrag
#+caption: Betrag der Magnetfeldvektoren. Die angelegten Magnetfeldsequenzen sind in grün und orange dargestellt, der Betrag der Magnetfeldvektoren ist in grün dargestellt.
#+attr_latex: :width \textwidth :placement [h]
