...
#+name:fig-force_spp
#+caption: Schematische Darstellung der wirkenden Kräfte auf ein superparamagnetisches Partikel oberhalb des EB-Dünnschichtsystems. Zu sehen sind van-der-Waals-Kraft $F_\mathrm{vdw}$, elektrostatische Kraft $F_\mathrm{e}$ ,magnetostatische Kraft $F_\mathrm{m}$, Reibungskraft $F_\mathrm{R}$ und magnetische Kraft $F_\mathrm{M}$. Diese resultieren zusammen jeweils in die Gleichgewichtsgeschwindigkeit $v_\mathrm{SPP}$ und den Gleichgewichtsabstand $z$. Die Längen der Kraftvektoren sind zu Anschauungszwecken arbiträr gewählt.
...
#+begin_src python :session mag :results output
  ...
  def B_helmholtz_fixed(x, U, c):
      ...
      n = 660
      r = 0.030
      b = constants.mu_0 * (8 * I_fixed(x, sr=48000) *
                                  n) / (np.sqrt(125) * r) + c
      return b
...
#+begin_src python :session mag :results output
  ...
  for i, data in enumerate(avglist):
      ...
      if i < 2:
          ...
          ax[0, i].legend()
          ax[0, i].plot(I_fixed(x) * 10.34 / 100, label="test")
      else:
          ax[1, i - 2].plot(data[:cut], label="Messwerte")
          ax[1, i - 2].plot(
              B_helmholtz_fixed(x, *popt),
