...
#+begin_src python :session strom :results output
  ...
  for i, data in enumerate(avglist):
      ...
      popt, pcov = curve_fit(I, x, data[:cut], p0=(1,R_z,L_z), bounds=(0,[40,25,.05]), method="trf")
      if i < 2:
          ax[0,i].plot(data[:cut], '.', markersize=2, label="Messwerte")
          ax[0,i].plot(I(x,*popt), 'r--', label=f"Fit: U={popt[0]:.3}, R={popt[1]:.2}, L={popt[2]:.2}")
          ax[0,i].legend()
      else:
          ax[1,i-2].plot(data[:cut], '.', markersize=2, label="Messwerte")
          ax[1,i-2].plot(I(x,*popt), 'r--', label=f"Fit: U={popt[0]:.3}, R={popt[1]:.2}, L={popt[2]:.2}")
          ax[1,i-2].legend()
  ax[1,0].set_xlabel("Datenpunkte")
  ax[1,1].set_xlabel("Datenpunkte")
  ax[0,0].set_ylabel("$I$ [A]")
...
#+begin_src python :session mag :results output
  ...
  def B_helmholtz(x, U, R, L, c):
    ...
    """
      n = 330
      r = 0.030
      b = 4 * np.pi * 10**(-7) * (8 * I(x, U, R, L, sr=48000) *
                                  n) / (np.sqrt(125) * r) + c
      return b
  ...
  for i, data in enumerate(avglist):
      ...
      popt, pcov = curve_fit(B_helmholtz,
                             ...
                             data[:cut],
                             p0=(1, R_z, L_z, 1),
                             bounds=([0, 0, 0, -20], [50, 50, 5, 20]),
                             method="trf")
      if i < 2:
          ax[0,i].plot(data[:cut], label="Messwerte")
          ax[0, i].plot(B_helmholtz(x, *popt),
                        'r--',
                        label=f"Fit: U={popt[0]:.3}, R={popt[1]:.2}, L={popt[2]:.2}")
          ax[0,i].legend()
      else:
          ax[1,i-2].plot(data[:cut], label="Messwerte")
          ax[1, i-2].plot(B_helmholtz(x, *popt),
                        'r--',
                        label=f"Fit: U={popt[0]:.3}, R={popt[1]:.2}, L={popt[2]:.2}")
          ax[1,i-2].legend()
