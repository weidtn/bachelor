...
#+Options: toc:2 tasks:t title:nil d:nil
#+Todo: TODO(t) | DONE(d) 
#+EXCLUDE_TAGS: ignore
#+LANGUAGE: de
...
** TODO Néel-Wände
** TODO Superparamagnetismus Zeitskala
** TODO Suszeptibilität
* Header                                                             :ignore:
   #+latex_class:scrbook
...
:Code:
Imports
#+begin_src python :session spule :results output :exports none
  import numpy as np
  import matplotlib
  matplotlib.use("Agg")
...
#+begin_src python :session spule :results output :exports none
  ...
  axes[0].set_ylabel("$U_\mathrm{0}$ [V]")
  axes[0].set_xlabel("$t$")
  ...
  # axes[1].legend()
  plt.tight_layout()
  plt.savefig("./img/einschalten.pdf", dpi=600)
#+end_src

#+RESULTS:

[[file:./img/einschalten.pdf]]
...
#+begin_src python :session spule :results output :exports none
  ...
  # axes[1].legend()
  plt.tight_layout()
  plt.savefig("./img/ausschalten.pdf", dpi=600)
#+end_src

#+RESULTS:

[[file:./img/ausschalten.pdf]]
:END:
...
\end{subfigure}
\caption{Schematische Darstellung der Stromstärke und Spannung bei Ein- und Abschaltvorgang in einem Stromkreis mit Widerstand $R$, Induktion $L$ und einer Diode um einen Weg für den Abschaltinduktionsstrom zu liefern. Ein- bzw. Abschalten geschehen dabei zum Zeitpunkt $t=0$.}
\end{figure}

** TODO [0/5] Magnetismus in Materie label:sec-magnetismus
...
# Paramagnetismus:
# ------
Ein paramagnetischer Festkörper besitzt bereits ohne äußeres Magnetfeld magnetische Momente. Jedoch ist die Richtung der magnetischen Momente durch die thermische Energie in alle Raumrichtungen verteilt, und die magnetischen Momente wechselwirken untereinander nicht. In Summe ist makroskopisch keine Magnetisierung erkennbar:cite:Demtroder2017
...
*** TODO Exchange Bias Effekt label:sec-EB
    Der Exchange Bias Effekt wurde 1956 von \textsc{Meiklejohn} und \textsc{Bean} an oxidierten \ce{Co}-Partikeln entdeckt.cite:Meiklejohn1957
