...
#+begin_src python :session strom :results output
  ...
  for i, data in enumerate(avglist):
      ...
      r2 = 1 - (ss_res / ss_tot)
      print(f"")
      t  = np.linspace(0,500/48000,len(data[:cut]))
      if i < 2:
...
:   File "c:/Users/EXP4-A~1/AppData/Local/Temp/babel-SyfopL/python-EozkAu", line 30, in <module>
:     traj_matrix = np.reshape(particle, (len(particle), 17))
:   File "C:\Users\exp4-AGE1-106\AppData\Roaming\Python\Python37\site-packages\numpy\core\fromnumeric.py", line 292, in reshape
...
#+begin_src python :session magnitude :results output
  ...
  plt.clf()
  fig, axes = plt.subplots( len(slope1), 1, figsize=(5,15), sharex=True)
  for idx in range(len(slope1)):
      axes[idx].scatter(t, x_trap[idx], s=0.1, label = "x")
      axes[idx].scatter(t, z_trap[idx], s=0.1, label = "z")
      axes[idx].scatter(t, mag[idx], s=0.1, label = "magnitude")
      # axes[idx].set_ylim(-1)
