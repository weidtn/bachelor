...
#+begin_src python :session strom :results output
  ...
  popt0, pcov0 = curve_fit(I, np.linspace(0,len(avglist[0]),cut), avglist[0][:cut], p0=(1,R_z,L_z), bounds=(0,[10,25,5]), method="trf")
  plt.clf()
  fig, ax = plt.subplots(2,2)
  ax[0,0].plot(avglist[0][:cut])
  ax[0,0].plot(I(np.linspace(0,len(avglist[0]),cut), *popt0), 'r--')
