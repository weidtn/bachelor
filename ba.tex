...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{img/force_spp.png}
\caption{\label{fig-force_spp}
Schematische Darstellung der wirkenden Kräfte auf ein superparamagnetisches Partikel oberhalb des EB-Dünnschichtsystems. Zu sehen sind van-der-Waals-Kraft \(F_\mathrm{vdw}\), elektrostatische Kraft \(F_\mathrm{e}\) ,magnetostatische Kraft \(F_\mathrm{m}\), Reibungskraft \(F_\mathrm{R}\) und magnetische Kraft \(F_\mathrm{M}\). Diese resultieren zusammen jeweils in die Gleichgewichtsgeschwindigkeit \(v_\mathrm{SPP}\) und den Gleichgewichtsabstand \(z\). Die Längen der Kraftvektoren sind zu Anschauungszwecken arbiträr gewählt.}
\end{figure}
...
Um die Validität der Näherung für die magnetische Flussdichte von Helmholtzspulen in Abhängigkeit der Stromstärke (Gleichung \ref{eq-helmholtz}) für die verwendeten Spulen zu überprüfen, wurde die Gleichung nach einigen mathematischen Umformungen.
